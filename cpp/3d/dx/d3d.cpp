...
//
///////////////////////////////////////////////////////////
HRESULT InitD3D(IDirect3D9 **ppD3D,IDirect3DDevice9 **ppD3DDevice,
                HWND hWnd,DWORD Width,DWORD Height,DWORD Depth,	
				BOOL WindowMode,BOOL ForceWindowed,BOOL	MultiThreaded)
{
	...
	*ppD3D = pD3D;
	...
	if(FAILED(hr = pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, Flags,&d3dpp, &pD3DDevice)))
	{
		if(FAILED(hr = pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF, hWnd, Flags,&d3dpp, &pD3DDevice)))
		{
			return hr;
		}
	}
	...
	*ppD3DDevice = pD3DDevice;
	...
	D3DXMatrixPerspectiveFovLH(&matProjection, D3DX_PI/4.0f, Aspect, 1.0f, 1000000.0f);
                         ...
                         IDirect3DVertexDeclaration9 **ppDecl)
{
	HRESULT hr;
	 ...
	 //LPCWSTR
	 // Error checking
	 if(!ppShader || !pDevice || !Name)   return E_FAIL;

	// Load and assemble the shader
	ID3DXBuffer *pCode;
	if(FAILED(D3DXAssembleShaderFromFile(Name, NULL, NULL, 0, &pCode, NULL)))
	{
		if(FAILED(D3DXAssembleShaderFromResource(NULL,Name, NULL, NULL, 0, &pCode, NULL)))
		{
   			return E_FAIL;
		}
	}
...
bool CreateTextureFromResource(LPDIRECT3DDEVICE9 pD3DD,
		LPCTSTR pSrcResource,LPDIRECT3DTEXTURE9 *ppTexture,HMODULE hSrcModule)
{
    HRSRC                 hres          = NULL;
    void*                 pMem          = NULL;
    DWORD                 dwSize        = 0;
    ...
    // Find the resource
    hres = FindResource( hSrcModule,pSrcResource,"TEXTURE" );
    if( NULL == hres ) 
        return false;

    // Load the resource
    pMem = (void*)LoadResource(hSrcModule, hres );
    if( NULL == pMem ) 
        return false;

    // Store the size of the resource
    dwSize = SizeofResource( hSrcModule, hres ); 


	if( FAILED(D3DXCreateTextureFromFileInMemoryEx(pD3DD,pMem,dwSize ,
