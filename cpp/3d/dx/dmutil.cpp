...
bool InitAudio(CMusicManager **ppMM, HWND hWnd)
{
	
	CMusicManager* pDXMM;
    // Create the music manager class, used to create the sounds
    pDXMM = new CMusicManager();
    if( FAILED( pDXMM->Initialize( hWnd ) ) )
	{
        return false;
	}
    ...
    pDXMM->SetSearchDirectory( szPath );
                                                  ...
                                                  BOOL bIsMidiFile )
{
    ...
    // Find the resource
    hres = FindResource( hmodule,strResource,strResourceType );
    if( NULL == hres ) 
        return E_FAIL;

    // Load the resource
    pMem = (void*)LoadResource( hmodule, hres );
    if( NULL == pMem ) 
        return E_FAIL;
